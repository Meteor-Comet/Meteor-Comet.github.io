...
## 14. 全球性能第一的微型 ORM：Dapper 详细使用指南

**官方/GitHub地址**：[https://github.com/DapperLib/Dapper](https://github.com/DapperLib/Dapper)

Dapper 由 StackOverflow 团队开发并开源。它是市面上名副其实**性能最高的 ORM（仅比纯原生的 Ado.Net 慢少许，完全秒杀 EF Core 等重型框架）**。
Dapper 的理念是“微小轻量”，它是一个扩展在 `IDbConnection` 接口上的库，**完全不负责 SQL 拼写逻辑，而是将结果集（Reader）映射为 C# 对象的底层引擎**。

如果你或你的公司有严苛复杂的 SQL（几百行连表报表查询），在 EF Core 或 SqlSugar 编写 Lambda 觉得费劲，并且极度在乎查询的响应耗时，那么 Dapper 是你的不二之选。

### 14.1 引入包与核心理念

通过 NuGet 安装包：`Dapper`。如果用的是 SqlServer，一并安装 `Microsoft.Data.SqlClient`。

Dapper 非常小巧，一共就只有 `Query` 和 `Execute` 几个扩展方法。

### 14.2 Dapper 基础实战演示（原生方式）

为了使用 Dapper，你需要自己来控制 `SqlConnection`，并且自己写 原汁原味的 SQL 文本字符串：
...
{
    private string _connectionString = "Server=(local);Database=TestDb;UID=sa;PWD=123";
    ...
    public List<UserEntity> GetAllUsers()
    {
        using (IDbConnection db = new SqlConnection(_connectionString))
        {
            ...
            // Dapper 会自动根据字段名称反射匹配给 UserEntity 类，名称不区分大小写
    ...
    public UserEntity GetUserByName(string username)
    {
        using (IDbConnection db = new SqlConnection(_connectionString))
        {
            // 使用 @username 占位，利用匿名对象的成员去传参。
            // Dapper 底层会自动转化为 SqlParameter，100% 杜绝 SQL 注入漏洞！
            string sql = "SELECT * FROM SysUser WHERE UserName = @username";
            
            // QueryFirstOrDefault 如果找不到就返回 null
            return db.QueryFirstOrDefault<UserEntity>(sql, new { username = username });
        }
    }
    ...
    public int InsertUser(UserEntity user)
    {
        using (IDbConnection db = new SqlConnection(_connectionString))
        {
            string sql = "INSERT INTO SysUser (UserName, CreateTime) VALUES (@Name, @CreateTime)";
            // 直接把整个实体扔进去当参数，Dapper 会自动去抽离里面同名的公有属性
            return db.Execute(sql, user);
        }
    }

    // 4. In 语法动态数组查询魔术
    public List<UserEntity> GetManyByInQuery(int[] searchIds)
    {
         using (IDbConnection db = new SqlConnection(_connectionString))
         {
             // 传统 ADO.Net 写 In 操作拼接字符串是噩梦
             // 但对于 Dapper，只要变量是一个数组/集合，你直接传进去，它会自动解析成 IN (@id1, @id2...)
             string sql = "SELECT * FROM SysUser WHERE Id IN @ids";
             return db.Query<UserEntity>(sql, new { ids = searchIds }).ToList();
         }
    }
}
```

### 14.3 WPF中的现代注入封装（整合 IConfiguration 与 DI）

在前面第 12 章节讲解过全局依赖注入，现在我们通过依赖注入模式，在 `App.xaml.cs` 中实现 Dapper 仓储的配置。对于 Dapper 而言，我们不再注册复杂的上下文对象，而是直接**注册提供连接字符串的连接工厂服务或直接注入连接符**。
...
services.AddTransient<IDbConnection>(sp => 
{
    ...
    // 每次索要 IDbConnection 时，都会送你一个新的数据库连接实例
    return new SqlConnection(connString);
});

// 然后再注入我们自己写的业务逻辑使用
services.AddTransient<IUserService, DapperUserService>();
```

**在服务层中使用（采用异步模式 `*Async` API）：**
...
public class DapperUserService : IUserService
{
    ...
    public DapperUserService(IDbConnection dbConnection)
    {
        _dbConnection = dbConnection;
    }

    // Dapper 天然完全支持 async / await 异步操作。这对 WPF 界面不卡死至关重要。
    public async Task<int> UpdateUserNameAsync(int userId, string newName)
    {
        string sql = "UPDATE SysUser SET UserName = @newName WHERE Id = @userId";
        
        // 我们不需要写 _dbConnection.Open()。
        // Dapper 有个智能机制，如果你传给它的 Connection 是 Closed 的，
        // 执行前会自动打开它执行，完毕后自动关闭它（释放回池中）！极其省心。
        int rowsInfo = await _dbConnection.ExecuteAsync(sql, new { newName, userId });
...
在 WPF 的开发框架选型时，往往在数据库技术栈上会有一场团队“血案”。这里给出一个标准客观的抉择建议：

| 特性评估项 | SqlSugar | Dapper |
| :--- | :--- | :--- |
| **性能测试** | 中上 (优于 EFCore) | **极快** (贴身肉搏 Ado.Net) |
| **学习门槛** | 极低（纯中文文档，非常符合国人思维模式） | 中低（需要掌握手写原生高性能复杂的 SQL 的能力） |
| **多库支持** | **极强** (自动翻译方言) | 弱（本身没有翻译器，如果手写 SqlServer 方言，切成 MySQL 会立刻崩溃） |
| **复杂对象/树结构查** | 原生拥抱导航属性、连表全自动装配 Lambda | 查可以，装配需要自己手写 Map，极度复杂 |
| **应用建议场景** | **企业中后台 ERP、OA、复杂连表业务开发** | **超高并发接口、性能瓶颈极度苛刻、报表查询、底层引擎设计** |

最后：这两者并不是你死我活的关系。在超大型应用中，我们非常推荐**读写分离**或**架构分离**：对于修改、插入、连表的普通业务使用 SqlSugar，对于复杂的百万数据级连卡报表联查统计接口，只利用 Dapper 手拼 SQL 返回结果集。
