...
- [21. 工业数据云端化：ASP.NET Core WebAPI 构建实战](#21-工业数据云端化asp-net-core-webapi-构建实战)
...
## 21. 工业数据云端化：ASP.NET Core WebAPI 构建实战

在 OT（操作技术）与 IT（信息技术）融合的趋势下，仅具备底层协议（Modbus/MQTT）的通讯能力已不足以支撑现代化的工业应用。WebAPI 作为基于 HTTP 协议的标准化接口，是连接现场层数据与企业级应用（如 ERP、MES）或前端看板的核心纽带。

### 21.1 为什么在工业网关中集成 WebAPI

1.  **标准化解耦**：JSON 格式已成为跨语言通信的事实标准。通过 WebAPI，无论上位机是使用 Java、Python 还是现代前端框架（Vue/React），都能以一致的方式调取实时参数。
2.  **防火墙穿透性**：HTTP(S) 通常在企业网络防火墙中是被允许的，相比复杂的 Socket 长连接，WebAPI 开发和部署门槛更低。
3.  **无状态查询**：对于非实时控制的报表查询、系统配置修改，请求/响应模式比轮询订阅模式更节省资源。

### 21.2 现代架构选型：Minimal APIs

在高性能工业网关或小型服务中，推荐使用 .NET 6+ 引入的 **Minimal APIs** 架构。相比传统的 Controller 模式，它具有更低的内存开销和更快的启动速度，非常适合部署在嵌入式或边缘计算节点。
...
// 注册 OpenAPI (Swagger) 用于生成接口文档
builder.Services.AddEndpointsApiExplorer();
builder.Services.AddSwaggerGen();

var app = builder.Build();

// 启用 Swagger UI，方便调试人员直接在浏览器测试指令
...
// 模拟工业现场数据存取
var sensorData = new { DeviceId = "S001", Temperature = 25.6, Status = "Normal" };

// 定义 GET 操作：读取实时遥测数据
app.MapGet("/api/telemetry/{id}", (string id) => {
    // 实际项目中此处应从 DataStore 或数据库中提取
    return Results.Ok(sensorData);
});

// 定义 POST 操作：下发控制指令
app.MapPost("/api/control", (ControlCommand cmd) => {
    // 解析 JSON 指令并转换回底层协议（如 Modbus）下发
    System.Console.WriteLine($"收到下发指令: {cmd.CommandName} 到 {cmd.Target}");
    return Results.Accepted();
});

app.Run();

public record ControlCommand(string Target, string CommandName, int Value);
```

### 21.3 工业级 WebAPI 的关键约束建议

#### 1. 序列化性能优化
在处理每秒数千次的 API 调用时，应当使用 `System.Text.Json` 替代 `Newtonsoft.Json`。后者由于使用了大量的反射机制，在高性能场景下是内存分配的主要诱因。

#### 2. 全连接状态同步
由于 WebAPI 是无状态的，它通常无法感知底层物理设备是否在线。因此，在返回数据包时，务必包含 `Timestamp`（采集时间戳）以及 `Quality`（数据质量码，如：0=失效, 1=有效），防止前端展示已经过期的残留缓存数据。

#### 3. 异步非阻塞架构
所有的 WebAPI 路由处理函数（Handlers）应该全部采用 `async/await`。严禁在接口内部执行阻塞式的 `Socket.Read` 或同步的 `Thread.Sleep`，这会导致 ASP.NET 的线程池耗尽，从而引发接口大面积超时反溃。

#### 4. 安全防护：API Key 与速率限制
工业设备接口直接暴露在网络中极其危险。至少应在端点级别实现基于 API Key 的校验逻辑，并启用 `RateLimiting`（限流中间件），防止恶意脚本发起的暴力轮询将网关内存打爆。
