...
本手册旨在为开发人员提供关于 BoTech 工业自动化控制软件框架的深度总结。内容涵盖 UI 界面参数配置映射、后台 Excel/XML 数据字典架构、自动化工站流程开发 SOP、状态控制逻辑，以及全部常用 API 函数的详细参数、返回值与代码示例。
...
2. [UI 界面参数配置映射与数据库架构](#2-ui-界面参数配置映射与数据库架构)
   - [2.1 UI 界面参数标签页 (TabPage) 绑定关系](#21-ui-界面参数标签页-tabpage-绑定关系)
   - [2.2 Excel 参数配置与 XML 数据库转换映射规则](#22-excel-参数配置与-xml-数据库转换映射规则)
   - [2.3 硬件板卡、轴及 I/O 映射配置 (ParXlsx 核心设计)](#23-硬件板卡轴及-io-映射配置-parxlsx-核心设计)
   - [2.4 与 C# 枚举的绑定关系及致命注意事项](#24-与-c-枚举的绑定关系及致命注意事项)
...
框架提供了灵活、分层的参数配置系统，开发人员可通过参数页面对设备的工艺数据、传感器屏蔽开关进行无损配置。

### 2.1 UI 界面参数标签页 (TabPage) 绑定关系
...
#### 2.1.3 TabPage 4: Check Tab (功能使能复选框页)
...
### 2.2 Excel 参数配置与 XML 数据库转换映射规则

参数定义的源头位于 Excel 文件 `ParList.xlsx`，运行库直接从 `ParData.xml` 中读取数值和元数据。
...
#### 2.2.2 自动同步生成逻辑 (`AutoSetup`)
当在参数维护页面执行“AutoSetup”（自动建库）时，框架会读取 `ParList.xlsx`，并将每行数据转换为 `ParData.xml` 中的结构。如果 XML 中已有数据，则会无损重写元数据，并保持原有参数的当前值不变。
...
### 2.3 硬件板卡、轴及 I/O 映射配置 (ParXlsx 核心设计)

在设备方案与接线完成后，开发人员的第一步工作是根据电气原理图与 I/O 表，在 `D:\BZ-Parameter\RBF\ParXlsx\` 目录下配置硬件板卡、伺服轴、I/O 以及系统级参数。这一层直接构成了软件与物理硬件之间的通信桥梁。

#### 2.3.1 控制卡配置 (`CardPar.xlsx`)
该文件定义了系统中所安装的全部运动控制卡（如固高 GTS 卡等）及扩展板卡的型号与配置。
* **主要列定义**：
  * **编号**：板卡逻辑 ID（从 0 开始自增）。
  * **卡号**：物理板卡拨码设定的物理卡号。
  * **卡名称**：如 `DECAT` (固高主卡)、`GENEX` 等。
  * **供应商**：如 `固高` 等。
  * **起始序号** 与 **数量**：板卡所控轴或 IO 的起始地址和端口总数。
  * **主卡**：布尔值，是否为主控制卡（设置为 True 会自动载入并初始化底层的配置）。
  * **参数路径**：控制卡底层驱动配置文件（如 `+GTS_Config/gtn_core1.cfg`）的相对路径。系统在启动时，底层 DLL 会自动读取此路径的配置文件并进行板卡初始化。

#### 2.3.2 伺服轴配置 (`AxisPar.xlsx`)
该文件定义了系统中的物理伺服轴/步进轴，并将 C# 中的逻辑轴枚举映射到控制卡的物理通道上，同时完成脉冲当量的物理转换。
* **主要列定义**：
  * **编号**：轴的逻辑 ID。**必须**与 C# 源码 `EnumName.cs` 中 `mAxis` 枚举定义的顺序（如 `右X = 1`, `右Y = 2`）保持绝对一致。
  * **名称**：轴中文名称（如 `右X`）。
...
#### 2.3.3 数字 I/O 映射配置 (`Input.xlsx` 与 `Output.xlsx`)
配置物理传感器（光电开关、气缸磁簧开关、安全门）和物理输出动作（电磁阀、继电器、指示灯）。
* **`Input.xlsx` (输入) 与 `Output.xlsx` (输出) 主要列定义**：
  * **编号**：逻辑索引（从 0 开始）。必须与 C# `EnumName.cs` 中的 `InNo` 和 `OutNo` 枚举定义顺序完美一致。
  * **名称Cn** / **名称En**：信号在中英文界面上显示的名称。
  * **模块编号** / **序号**：指明该 I/O 信号线压接在第几张控制卡（或 IO 扩展卡）的哪一个物理引脚（Pin 0~15 或更大）上。
  * **初始状态**：系统复位上电时控制卡输出的默认状态（通常为 0。部分如电机刹车、气缸安全释放电磁阀等可初始化为 1，复位完毕后再交由逻辑控制）。
  * **原点序号** / **动点序号**（`Output.xlsx` 独有）：针对双控气缸，关联其缩回和伸出物理输入信号（`InNo`）的编号，用于气缸的防冲突等待和异常报警判定。

#### 2.3.4 系统参数配置 (`SysPar.xlsx`)
该文件定义了整台设备全局的硬件规格，用于系统启动时的内存初始化和边界保护。
* **主要参数名称与作用**：
  * **卡数量**：系统中控制卡及模块的总数（对应在内存中分配的 Card 对象数组大小）。
  * **轴数量**：设备拥有的轴总数。
  * **输入点数量** / **输出点数量**：数字量 I/O 的最大物理数量规格限制。
  * **设置参数数量**：参数列表 `mParList` 的最大容量上限（如 280 个）。
  * **轴控件数量**：UI 上实例化和展示的示教控制面板数。
  * **急停编号**：指定急停按钮所接的 `InNo` 输入索引，使系统底层的急停监控线程能够高频获取该 DI。一旦触发，立刻挂起全部轴并关闭所有输出，实现硬件级防碰撞。

---

## 2.4 与 C# 枚举的绑定关系及致命注意事项

在 C# 流程开发中，我们绝不能使用裸的物理通道号（如 0, 1, 2）或硬编码变量，而是使用 `EnumName.cs` 里的枚举。

> [!CAUTION]
> **致命注意事项（枚举一致性校验）**：
> 1. C# 源码中的 `Enum` 值在执行时会直接强转为 `short` 索引（如 `(short)InNo.流线2到位信号`），去 `mFunction.m_Input` 数组中提取卡号和通道号。
> 2. 因此，C# 中的枚举排序（项的顺序）**必须**与 Excel 表中的“编号”顺序保持 100% 绝对一致。
> 3. 如果在 Excel 中对 I/O 或轴进行了删减或移位，导致 `编号` 改变，而 C# 代码中的枚举项未作同步更新，编译器**不会报错**，但在自动运行时，程序将读取或置位错误的物理引脚。这将极易导致气缸硬撞击、机械臂偏置超程，甚至引发生命安全事故！
> 4. 每次修改 Excel 表后，必须执行软件的 `AutoSetup`（自动建库生成XML）工具，重新生成对应的 `ParInput.xml`、`ParOutput.xml`、`ParMachine.xml` 和 `SysPar.xml`，重启软件后，新的硬件映射才真正生效。
...
3. **输入反馈 (DI 实时点亮)**：
   系统会在后台开启一个 10ms 级别的扫描线程，高频读取 `ParInput.xml` 里配置的所有卡号和引脚状态。一旦传感器触发（引脚变高电平），UI 监控上对应的指示灯会点亮成绿色；离开后熄灭。这为电气调试和故障排查提供了极其便捷的可意化支持。
...
2. **上下限拦截**：当输入的值 $V > LimitUp$ 或 $V < LimitDown$ 时，界面会弹出警告对话框，或直接在失去焦点时将数值限制在边界值，拒绝写入 XML，保证底层设备动作的绝对安全。
...
所有工站控制类必须遵循本节定义的生命周期函数 and 状态机规范进行开发。
...
通信端口编号（如 `TCPIP_Port` 或 `RS232_Port`）定义在 `mEnum.cs` 中。
...
```csharp
// 重新开始计算扫码等待时间，防止因为流入和到位夹紧动作导致耗时，进而引发虚警超时
mFunction.ConveyorData[MainConvId].StartTime = mFunction.GetTickCount(); 
