...
2. [UI 界面参数配置映射与数据库架构](#2-ui-界面参数配置映射与数据库架构)
   ...
   - [2.2 Excel 配置与 XML 数据库转换映射规则](#22-excel-配置与-xml-数据库转换映射规则)
   - [2.3 输入限制与合法性校验逻辑](#23-输入限制与合法性校验逻辑)
   - [2.4 功能使能复选框在业务代码中的跳步与屏蔽机制](#24-功能使能复选框在业务代码中的跳步与屏蔽机制)
...
* **绑定控件**：由两个 `Zcm.PropertyParS` 控件构成：
...
### 2.2 Excel 配置与 XML 数据库转换映射规则

参数定义的源头位于 Excel 文件 `ParList.xlsx`，运行库直接从 `ParData.xml` 中读取。
...
### 2.4 功能使能复选框在业务代码中的跳步与屏蔽机制

在 Check Tab 中配置的布尔开关（`FuncChk`）直接参与 `Tasks` 中的时序控制。典型的屏蔽策略如下：

#### 2.4.1 扫码功能屏蔽 (`Enable_scanning_code` 索引 102)
...
#### 2.4.2 机械手关闭屏蔽 (`Block_leftRobot` 索引 116 / `Block_rightRobot` 索引 117)
...
#### 2.4.3 相机跳步逻辑 (`Spare7` 索引 107 - 启用左相机 / `Spare15` 索引 115 - 启用右相机)
...
所有工站控制类必须遵循本节定义的生命周期函数和状态机规范进行开发。
...
在多轴模组（如 X/Y/Z 三轴直角坐标机械臂）开发中，如果在运行状态下频繁调用 `pMove.WaitDone()` 或 `mDoDi` 进行手动轮询，容易产生以下问题：
