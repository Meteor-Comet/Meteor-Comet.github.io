...
> 本章内容来自 ZCM968SOP.docx 原文，详细说明各 UI 控件的配置与使用方法。

### 7.1 IO 控件

使用 IO 相关控件前，需要在 `D:\BZ-Parameter\[机型]\ParXlsx\` 目录下配置 `Input.xlsx` 和 `Output.xlsx` 文件。
...
配置 IO 相关控件时需注意左边编号与控件上的编号保持一致：

**[待添加控件图片]**

**[待添加控件图片]**

#### 7.1.1 StationRun 与工站状态

StationRun 控件用于展示和控制工站的运行状态，包括启停、手动/自动模式切换等功能。
...
使用 Cylinder 控件可以更方便地控制双控气缸。控件会对两个 IO 输出同时操作（关断一个并开启另一个），并可以根据 IO 输入状态判断气缸是否正常动作。

**[待添加控件图片]**

**[待添加控件图片]**

需要在控件的属性页面配置 `mInput1Index1`、`mInput2Index1`、`mOutIndex1`：
- `mInput1Index1`、`mInput2Index1` 为对应 IO 输入的编号
- `mOutIndex1` 为对应 IO 输出的编号
- 气缸双控需配置两个 IO，点击控件上的按钮会对应输出编号开启其中一个、关断另外一个
- 也可查看对应输入，判断气缸是否正常运动

#### 7.1.3 Zcm.Vacuum

使用 Vacuum 控件可以配置设备的真空吸附功能并查看真空状态。

**[待添加控件图片]**

**[待添加控件图片]**

需在控件的属性页面配置 `Input1` 和 `Output1`：
- `Output1` 为触发真空吸附 IO 对应的编号
- `Input1` 为检测真空信号对应的 IO 输入的编号
配置完成后，点击控件上的按钮可以判断是否触发真空吸附，堵住气孔后可以判断是否有真空信号。

#### 7.1.4 Zcm.mShp_Input

使用 mShp_Input 控件可以监测单个 IO 输入的状态。

**[待添加控件图片]**

**[待添加控件图片]**

需在控件的属性页面配置好 `mInputIndex1`（为对应的 IO 输入编号），配置完成即可使用。

#### 7.1.5 Zcm.mButton 与 mBtn_Out

**Zcm.mBtn_Out** 可以对单个 IO 输出进行使能和失能；**Zcm.mButton** 控件可以快速使能和失能单个 IO 输出。
...
需在控件的属性页面配置：
- `mOutIndex1`：IO 输出对应的编号
- `PlusWidth1`：根据需要可以配置此项来设置输出脉冲宽度，将输出方式从电平输出改为脉冲输出

#### 7.1.6 Zcm.InputClass 与 OutputClass

**Zcm.InputClass** 可以一次性显示多个 IO 输入的状态；**Zcm.OutputClass** 可以一次性显示多个 IO 输出的状态并提供使能和失能的方法。
...
需要配置 `mCardIndex1` 和 `mCtlIndex1`：
- `mCardIndex1`：为对应的卡编号
- `mCtlIndex1`：为对应的模块编号
一个控件显示 16 个 IO 状态，根据卡编号和模块编号自动索引到相关的 IO 项。
...
同样需要在属性页面配置 `mCardIndex1` 和 `mCtlIndex1`，配置完成后点击按钮验证配置是否正常。

### 7.2 轴相关控件

#### 7.2.1 Zcm.Teach

使用 Teach 控件可以更便利地操作运动轴。在控件的配置页面需要配置：
- `AxisNum_X`、`AxisNum_Y`、`AxisNum_Z`、`AxisNum_R`：若没有对应轴则配置为 0，有对应轴则配置对应的轴编号
- 还可以通过改变属性中的 Z 安全高度，为模组 Z 轴设置安全高度

**[待添加控件图片]**

**[待添加控件图片]**

**轴使能与回原点 (Enable Origin)：**
在 Enable Origin 页面可以查看各个轴的状态，也可以对每个轴进行单独的使能或回原点操作。

**[待添加控件图片]**

**轴手动控制 (Manual Control)：**
使能且回原点后可以在 Manual Control 页面自由移动模组：
- 自由移动方式分为长距 (Long)、中距离 (Medium)、短距离 (Short)
- 也可在 TextBox 自定义 (Custom) 设置移动距离
- 选择连续 (Continuous) 则可以长按持续移动
- 还可以通过 Stop 按钮打断当前动作

**[待添加控件图片]**

**点位示教与保存 (PointData)：**
在 PointData 页面可以对点位进行增删改查：
- 在 Manual Control 页面切换到执行运动页面
- 通过下拉菜单选择对应的点位
- 点击运动按钮并确认后，即可使模组移动到对应的点位

**[待添加控件图片]**

**[待添加控件图片]**

#### 7.2.2 Zcm.TeachXY / TeachS / motorStatus

**Zcm.TeachXY**：具体使用方法与 Teach 类同，适用于只有两个轴的模组，较为节约空间。

**[待添加控件图片]**

**Zcm.TeachS**：具体使用方法与 Teach 类同，适用于只有 1 个轴的模组，较为节约空间。

**[待添加控件图片]**

**Zcm.motorStatus**：使用 Zcm.motorStatus 控件可以监控多个轴的状态。需要在控件的属性页面配置卡号和轴号，配置完成后可以在此页面监控各个轴的状态。
...
Zcm.Robot 控件用于控制四轴或六轴机械手。在控件的属性页面需要配置多项内容：

**[待添加控件图片]**

**[待添加控件图片]**

**输入设定**：配置好 Robot 的输入设定相关 IO，以便监测机械手运行状态。

**输出设定**：输入设定配置完成后，还需配置机械手的输出设定，通过对应的 IO 输出可以控制机械手的启停、复位等相关操作。

**点位数量**：配置属性中的点位数量可以控制控件中点位数据页面显示的数据条数。

**[待添加控件图片]**

**换位相关属性**：可以交换轴正负移动按钮的位置。

**RobotSelect**：最后还需选择属性中的 RobotSelect，确定机械手是四轴还是六轴。

**[待添加控件图片]**

配置完成后启动机械手，即可在手动控制页面控制机械手的移动。

**PointData 界面**：在控件的 PointData 界面可以查看机械手存取的点位，并对点位进行增删改查（此处点位和机械手处的点位文件对应）。

**[待添加控件图片]**

**IOStatus 页面**：在 IOStatus 页面可以查看 Robot 的各种状态，也可以对机械手进行启停、复位等控制。

**[待添加控件图片]**

**点位控制**：下拉点位菜单项可以选择对应的点位，选择后点击运动按钮并确认弹出的弹窗后，Robot 便会移动到对应的点位。

**[待添加控件图片]**

**手动保存点位 (Operation)：**
在 Operation 页面可以通过手动控制将机械手移动到需要的位置：
- 将点位菜单选择到需要保存的点位项
- 勾选 Permit 的 CheckBox 后点击 Save 按钮
- 点击确认后将当前点位保存到对应的点位中
- 如果保存的点位与之前的点位偏差过大，则需要输入密码才可以保存（密码一般为 1234）
...
使用参数相关控件前，需要在 `D:\BZ-Parameter\[机型]\ParXlsx\` 目录下配置 `ParList.xlsx` 文件。
...
配置参数相关控件时需注意左边序号与控件上的编号保持一致。

#### 7.4.1 Zcm.PropertyChk / PropertyChkS

使用 Zcm.PropertyChk 控件可以对多个参数进行查改。需要配置控件的 `StartIndex` 和 `ParNumber` 两项：
- `StartIndex`：参数列表的起始索引
- `ParNumber`：显示的参数量

配置完成后会根据参数列表自动在对应的位置读取参数项，在控件上修改参数后点击 Save 按钮即可将修改后的数据进行保存。
...
使用 Zcm.UserChk 控件可以对多个 bool 类型参数进行查改。配置方式与 PropertyChk 相同，需要配置 `StartIndex` 和 `ParNumber` 两项。

**[待添加控件图片]**

**[待添加控件图片]**

#### 7.4.3 Zcm.UserPar / UserParS

使用 Zcm.UserPar 控件可以对多个参数进行查改。需要配置控件的 `StartIndex` 和 `ParNumber` 两项。
...
#### 7.4.4 Zcm.Tcp_IP（通讯配置）

使用 TCP_IP 控件可以配置设备的 TCP/IP 通讯参数。需要在控件的属性页面修改属性中的 `PortID`。
...
在控件上输好具体的 IP 和 Port 后点击 Save 按钮，对应的 IP 和端口会存储到本地。程序加载时会读入本地存储的文件，可通过相应方法访问保存的 IP 和端口。

### 7.5 其他控件

#### 7.5.1 Zcm.Converoy

使用 Zcm.Converoy 控件可以控制流水线（传送带）的运行。需在属性页面配置 `StationID`（为对应的流水线编号），配置完成后通过此控件即可控制流线的启停相关操作。

**[待添加控件图片]**

#### 7.5.2 Zcm.Stats

使用 Zcm.Stats 控件可以显示设备的运行统计信息。在控件属性页面可以对显示行数进行编辑，调整需要的显示行数。
